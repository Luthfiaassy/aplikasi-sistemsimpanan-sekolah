...
$kode=$_GET['nip'];
$nama=mysql_fetch_array(mysql_query("SELECT nama_guru from tb_guru where nip='$kode'"));
...
$pdf->Cell(25.2,0.7,"Laporan Setoran Guru ".$nama['nama_guru']."",0,10,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
...
$query = mysql_query("SELECT * from tb_setroguru where nip='$kode'");
$no=1;
while($data=mysql_fetch_array($query)){
    ...
    $pdf->Cell(4, 0.8, $data['u_entry'], 1, 0,'C');
     $pdf->Cell(4, 0.8, $data['tgl_entri'], 1, 1,'C');$no++;}
$hasil=mysql_fetch_array(mysql_query("SELECT sum(besar_simpanan) as besar from tb_setroguru where nip='$kode'"));
$pdf->Cell(15, 0.8, 'Total', 1, 0, 'C');
$pdf->Cell(8, 0.8, number_format($hasil['besar']), 1, 0, 'C');

$pdf->Output("Laporan setoran Guru.pdf","I");
